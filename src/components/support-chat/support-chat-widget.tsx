...
export function SupportChatWidget() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (!openRef.current) {
        const newModeratorMsgs = d.messages.filter(
          (m) => m.senderType === "moderator",
        ).length;
        ...
        prevMessageCountRef.current = d.messages.filter(
          (m) => m.senderType === "moderator",
        ).length;
      }
    } catch {
  ...
  useEffect(() => {
    const interval = setInterval(
      fetchData,
      open ? 5000 : data?.chatConsentGiven ? 5000 : 20000,
    );
    return () => clearInterval(interval);
  }, [fetchData, open, data?.chatConsentGiven]);
  ...
  return (
    <>
        ...
        >
          <div className="overflow-hidden rounded-l-2xl rounded-tr-2xl border border-primary/10 bg-white/95 shadow-xl backdrop-blur-md">
            {data?.social.telegram && (
              ...
              >
                <svg className="h-5 w-5 text-[#229ED9]" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.95-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.36.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .38z"/></svg>
                Telegram
              </a>
            )}
            {data?.social.max && (
              ...
              >
                <svg className="h-5 w-5 text-[#168ACD]" viewBox="0 0 24 24" fill="currentColor"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H5.17L4 17.17V4h16v12z"/><path d="M7 9h2v2H7zm4 0h2v2h-2zm4 0h2v2h-2z"/></svg>
                MAX мессенджер
              </a>
            )}
            <button
              onClick={() => { setOpen(true); setHoverMenu(false); }}
      ...
      <button
        onClick={() => setOpen(true)}
        className={cn(
          "fixed right-4 z-[9998] flex h-16 w-16 items-center justify-center rounded-full shadow-lg lg:hidden",
          "bg-gradient-to-br from-primary via-primary/90 to-primary/80 backdrop-blur-sm",
          "transition-all duration-200 active:scale-95",
          open && "pointer-events-none scale-0 opacity-0",
        )}
        style={{ bottom: "calc(env(safe-area-inset-bottom, 0px) + 5.35rem)" }}
      >
      ...
      <div
        className={cn(
          ...
          // Mobile
          "inset-x-0 bottom-0 h-[88vh] rounded-t-2xl lg:inset-x-auto",
          // Shadow
          "shadow-[0_34px_90px_-40px_rgba(43,31,25,0.42)]",
          // Animation
          "transition-all origin-bottom-right",
          open
            ? "pointer-events-auto scale-100 opacity-100 duration-[320ms] ease-[cubic-bezier(0.22,1,0.36,1)]"
            : "pointer-events-none scale-95 opacity-0 duration-[180ms] ease-[cubic-bezier(0.22,1,0.36,1)]",
        )}
      >
        {/* Header */}
          ...
          <div className="border-t border-olive/5 bg-white px-4 py-2.5">
            <label className="flex cursor-pointer items-start gap-2 text-xs text-olive/60">
              ...
              <span>
                ...
                <a href="/consent" target="_blank" className="text-primary underline">
                  Подробнее
                </a>
              </span>
            </label>
