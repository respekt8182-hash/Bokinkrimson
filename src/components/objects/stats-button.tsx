...
const LS_PREFIX = "prop_stats_refresh_";
...
function StatsModal({
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/properties/${propertyId}/stats`);
  ...
  useEffect(() => {
    setRefreshRec(getRefreshRecord(propertyId));
    fetchData();
  ...
  function handleRefresh() {
    ...
    const next = bumpRefreshRecord(propertyId);
    setRefreshRec(next);
    fetchData();
  }
      ...
      <div className="relative z-10 w-full max-w-xl rounded-t-3xl bg-white shadow-2xl sm:rounded-2xl">
        ...
        <div className="flex items-start justify-between gap-3 border-b border-olive/10 px-5 py-4">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-xs text-olive/60">{propertyName}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
