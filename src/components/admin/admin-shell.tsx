...
export function AdminShell({ moderationSnapshot, currentAdmin, children }: Props) {
  ...
  const searchRecords = useMemo<SearchRecord[]>(() => {
    ...
    const utilityRecords: SearchRecord[] = [
      ...(hasAdminPermission(currentAdmin.role, "profile:manage")
        ? [
            {
              href: "/admin/profile",
  ...
  }, [currentAdmin.role, visibleNavigationGroups, visibleQuickActions]);
    ...
    <AdminShellContext.Provider value={currentAdmin}>
      <div className="admin-shell min-h-screen bg-[var(--admin-bg)] text-[var(--admin-text)]">
        ...
        >
          ...
          {visibleQuickActions.length > 0 ? (
            <div className="sticky top-[68px] z-20 border-b border-[var(--admin-border)] bg-[rgba(248,247,244,0.94)] px-4 py-2 backdrop-blur-xl sm:px-6 lg:hidden">
              <div className="-mx-1 flex gap-2 overflow-x-auto px-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                {visibleQuickActions.slice(0, 6).map((item) => {
                  const Icon = iconMap[item.icon];
                  return (
                    <Link
                      key={item.href}
                      href={item.href}
                      className="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl border border-[var(--admin-border)] bg-white px-3 text-sm font-semibold text-[var(--admin-muted)] shadow-[var(--admin-shadow-xs)] transition active:scale-[0.98] hover:border-[var(--admin-primary-border)] hover:text-[var(--admin-primary)]"
                    >
                      <Icon className="h-4 w-4" />
                      {item.label}
                    </Link>
        ...
        {searchOpen ? (
          <div className="fixed inset-0 z-50 bg-slate-950/32 p-3 backdrop-blur-sm lg:hidden">
            <div className="flex max-h-[calc(100dvh-1.5rem)] flex-col overflow-hidden rounded-2xl border border-[var(--admin-border)] bg-white shadow-[var(--admin-shadow-lg)]">
              <div className="flex items-center gap-2 border-b border-[var(--admin-border)] p-3">
                <div className="flex min-h-11 min-w-0 flex-1 items-center rounded-xl border border-[var(--admin-border)] bg-white px-3 shadow-[var(--admin-shadow-xs)] focus-within:ring-4 focus-within:ring-[var(--admin-ring)]">
                  <Search className="h-4.5 w-4.5 shrink-0 text-[var(--admin-faint)]" />
                  <input
                    ...
                    onKeyDown={(event) => {
                      if (event.key === "Enter") {
                        const target = searchResults[0] ?? searchRecords[0];
                        if (target) {
                          event.preventDefault();
                          openSearchResult(target.href);
                        }
                      }
                    ...
                    aria-label="Поиск по разделам админки"
                    placeholder="Найти раздел, оплату, модерацию..."
                    className="h-11 min-w-0 flex-1 border-0 bg-transparent px-3 text-sm text-[var(--admin-text)] outline-none placeholder:text-[var(--admin-faint)]"
                  />
                </div>
                <button
                  ...
                  className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-[var(--admin-border)] bg-white text-[var(--admin-muted)]"
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-2">
                {(normalizedSearchQuery ? searchResults : searchRecords.slice(0, 10)).length > 0 ? (
                  (normalizedSearchQuery ? searchResults : searchRecords.slice(0, 10)).map(
                    (item) => {
                      const Icon = iconMap[item.icon];
                      return (
                        <button
                          key={`mobile:${item.group}:${item.href}:${item.label}`}
                          type="button"
                          onClick={() => openSearchResult(item.href)}
                          className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition active:scale-[0.99] hover:bg-[var(--admin-primary-soft)]"
                        >
                          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--admin-muted-surface)] text-[var(--admin-primary)]">
                  ...
                  <div className="px-4 py-10 text-center">
                    <Search className="mx-auto h-7 w-7 text-[var(--admin-faint)]" />
                    <p className="mt-3 text-sm font-semibold text-[var(--admin-text)]">
                      Ничего не найдено
                    </p>
                    <p className="mt-1 text-sm text-[var(--admin-muted)]">
                      Попробуйте написать иначе: объект, отзывы, оплата, модерация.
                    </p>
                  </div>
                )}
              </div>
