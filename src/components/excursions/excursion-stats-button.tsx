...
const LS_PREFIX = "exc_stats_refresh_";
...
function bumpRefreshRecord(id: string): RefreshRecord {
  ...
  try { localStorage.setItem(LS_PREFIX + id, JSON.stringify(next)); } catch { /* noop */ }
  return next;
}
    ...
    <div>
      ...
      >
            ...
            <g key={d.date}>
              ...
              {d.count === 0 && (
                <line
                  ...
                  x2={x + barW} y2={PAD_T + PLOT_H}
                  stroke="#e0d6cc" strokeWidth={1}
                />
              )}
              {d.count > 0 && (
                <rect
                  x={x} y={y} width={barW} height={barH} rx={2.5}
              ...
              {isActive && (
                <g>
                  <rect
                    ...
                    width={92} height={20} rx={5}
                    fill="#2b1f19" opacity={0.93}
                  />
                  <text
        ...
        {pageData.map((d, i) => {
          ...
          return (
            <text key={d.date} x={x} y={VB_H - 6}
              textAnchor="middle" fontSize={7} fill="#9e8e82">
              {fmtDate(d.date)}
            </text>
          );
        })}
...
function ExcursionStatsModal({
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/excursions/${excursionId}/stats`);
  ...
  useEffect(() => {
    setRefreshRec(getRefreshRecord(excursionId));
    fetchData();
  }, [excursionId, fetchData]);
  ...
  function handleRefresh() {
    ...
    const next = bumpRefreshRecord(excursionId);
    setRefreshRec(next);
    fetchData();
  }
      ...
      <div className="relative z-10 w-full max-w-xl rounded-t-3xl bg-white shadow-2xl sm:rounded-2xl">
        ...
        <div className="flex items-start justify-between gap-3 border-b border-olive/10 px-5 py-4">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-xs text-olive/60">{excursionTitle}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-olive/50">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <AppIcon icon={RefreshCw} className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
                    Обновить
                  </button>
                </div>
              </div>
